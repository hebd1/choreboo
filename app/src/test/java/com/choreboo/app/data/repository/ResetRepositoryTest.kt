...
class ResetRepositoryTest {
    ...
    // ── clearLocalData uses explicit uid (D8 fix) ─────────────────────────────

    @Test
    fun `resetAll passes explicit uid to backgroundRepository clearLocalData`() = runTest {
        val testUid = "uid-explicit"
        val mockUser = mockk<FirebaseUser>(relaxed = true) {
            every { uid } returns testUid
            every { reauthenticate(any()) } returns Tasks.forResult(null)
            every { delete() } returns Tasks.forResult(null)
        }
        every { firebaseAuth.currentUser } returns mockUser
        ...
        // backgroundRepository.clearLocalData receives the uid captured BEFORE auth deletion
        coVerify { backgroundRepository.clearLocalData(testUid) }
    }

    // ── householdRepository.clearState is called ──────────────────────────────

    @Test
    fun `resetAll clears household state during local cleanup`() = runTest {
        val mockUser = mockk<FirebaseUser>(relaxed = true) {
            every { uid } returns "uid-household"
            every { reauthenticate(any()) } returns Tasks.forResult(null)
            every { delete() } returns Tasks.forResult(null)
        }
        every { firebaseAuth.currentUser } returns mockUser
    ...
    // ── syncManager.runExclusive wraps the entire operation ───────────────────

    @Test
    fun `resetAll runs inside syncManager runExclusive`() = runTest {
        every { firebaseAuth.currentUser } returns null
        val credential = mockk<AuthCredential>(relaxed = true)

        repo.resetAll(credential)

        coVerify { syncManager.runExclusive<ResetResult>(any()) }
    }
}
