...
class SyncManager @Inject constructor(
    ...
    suspend fun syncAll(force: Boolean = false): Boolean {
        ...
        // For forced syncs (post sign-in), pass force=true to getIdToken() — this triggers
        // a network round-trip that always suspends, guaranteeing the Data Connect SDK's
        // gRPC interceptor receives the refreshed token before any parallel calls launch.
        // This eliminates the race that previously required a 500 ms delay hack.
        // For background syncs (force=false), getIdToken(false) suffices — the token is
        // already cached and fresh.
        try {
            firebaseAuth.currentUser?.getIdToken(force)?.await()
        } catch (e: Exception) {
        ...
        return syncMutex.withLock {
            ...
            coroutineScope {
                ...
                // 6. Household data (best-effort, independent of habits)
                val householdDeferred = async {
                    try {
                        householdRepository.refreshAll()
                        true
                    } catch (e: Exception) {
                        Timber.e(e, "Household sync failed (non-fatal)")
                        false
                    }
                }
                ...
                householdDeferred.await()   // best-effort — don't gate anySuccess on it
